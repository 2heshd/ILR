...
- includes an opt-in 2,000-term news/formal-Persian catalog: 100 terms from a BBC Persian and Iran International frequency sample, 654 unique ChiMishe Newspaper Book terms, and 1,246 advanced formal-course terms
...
The API keys are used only in server routes; never expose either one with a `NEXT_PUBLIC_` prefix. Configure native Iranian Persian male and female voice IDs, and learners can switch between them under Account settings. Persian requires the `eleven_v3` model; Eleven Multilingual v2 does not support Persian. If an ElevenLabs voice is absent, the existing OpenAI speech path remains available as a deployment-safe fallback.
