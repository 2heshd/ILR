...
for(let repetition=1;repetition<=repetitions;repetition++)for(const level of [1,2,3,4])for(const kind of ["reading","listening"])for(const register of ["formal","colloquial"]){
  const bank=banks[level]; cases.push({repetition,kind,register,targetIlr:level,practiceSource:"topic",topic:bank.topic,targetWords:bank.words,wordDefinitions:[]});
}
...
const report={createdAt:new Date().toISOString(),base,repetitions,summary,results};
