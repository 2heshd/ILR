...
test('omitted labels are repaired only within the supporting-entry allowance',()=>{
  ...
  assert.ok(checkSupportingVocabulary('کتاب خانه شهر مدرسه معلم دانشجو دانشگاه کلاس خودکار دفتر میز صندلی فردا امروز بازار خرید غذا دوست خیابان دکان خریدم',[],[]).issues.length);
});
test('twenty-one entries and passage-sized declarations fail',()=>{
  assert.ok(checkSupportingVocabulary('',[],['کتاب','خانه','مدرسه','معلم','درس','شهر','دانشجو','دانشگاه','کلاس','خودکار','دفتر','میز','صندلی','فردا','امروز','بازار','خرید','غذا','دوست','خیابان','دکان']).issues.length);
