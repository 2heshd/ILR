-- Comprehension is an additional, per-class opt-in. Existing memberships stay private.
begin;
alter table public.learning_class_members add column if not exists share_comprehension boolean not null default false;

create or replace function public.learning_set_comprehension_sharing(target uuid, enabled boolean) returns void
language plpgsql security definer set search_path=public as $$
begin
  if auth.uid() is null then raise exception 'Sign in first'; end if;
  update public.learning_class_members set share_comprehension=coalesce(enabled,false) where class_id=target and user_id=auth.uid();
  if not found then raise exception 'Class membership required'; end if;
end $$;
revoke all on function public.learning_set_comprehension_sharing(uuid,boolean) from public,anon;
grant execute on function public.learning_set_comprehension_sharing(uuid,boolean) to authenticated;

-- Keep malformed/legacy snapshot fields from breaking a whole class report.
create or replace function public.learning_report_number(v jsonb) returns numeric
...
revoke all on function public.learning_report_number(jsonb) from public,anon,authenticated;
...
create or replace function public.learning_class_comprehension_report(target uuid) returns jsonb
language plpgsql security definer set search_path=public as $$
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('user_id',m.user_id,'comprehension_shared',m.share_comprehension,'comprehension',case when m.share_comprehension then public.learning_comprehension_evidence(s.state) else '[]'::jsonb end)),'[]') into result
  from public.learning_class_members m left join public.study_snapshots s on s.user_id=m.user_id where m.class_id=target;
  return result;
end $$;
revoke all on function public.learning_class_comprehension_report(uuid) from public,anon;
grant execute on function public.learning_class_comprehension_report(uuid) to authenticated;
commit;
