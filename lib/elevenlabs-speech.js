...
function speechUrl(withTimestamps) {
...
async function elevenLabsRequest(text, withTimestamps, signal) {
  const response = await fetch(speechUrl(withTimestamps), {
    method: "POST",
    signal,
    headers: {
...
export async function createElevenLabsSpeech(text, signal) {
  const response = await elevenLabsRequest(text, false, signal);
  const audio = Buffer.from(await response.arrayBuffer());
  if (audio.length < 500) throw new ElevenLabsSpeechError("ElevenLabs returned an empty audio file.", 502);
  return audio;
}

export async function createElevenLabsSpeechWithTimestamps(text, signal) {
  const response = await elevenLabsRequest(text, true, signal);
