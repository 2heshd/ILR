...
export function restoreCourseDefinitions(words, entries) {
  ...
  for (const entry of entries) {
    ...
    if (entry.id !== undefined && entry.id !== null) byId.set(String(entry.id), entry);
  }
  return words.map(word => {
    if (!word.courseEntryId) return word;
    const canonical = byId.get(String(word.courseEntryId));
    const displayForm = canonical?.fa || word.displayForm;
    const normalizedForm = normalizedKey({ displayForm });
    const definitions = byForm.get(normalizedForm);
    ...
    return definition === word.definition
      ...
      ? word
      : {...word,definition,displayForm,normalizedForm};
  });
}
