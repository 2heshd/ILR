...
export default function Classroom(){
  ...
  async function showReport(id:string){if(!client)return;const request=++reportRequest.current;setSelected(id);setBusy(true);setReport([]);try{const {data,error}=await client.rpc('learning_class_report',{target:id});if(error)throw error;if(request!==reportRequest.current)return;setReport(data??[]);setStatus('Last seven UTC calendar days · correct answers / attempts, measured separately for each skill. Select a student for details.');}catch{if(request!==reportRequest.current)return;setStatus('The class report could not load. Retry or check your class access.');}finally{if(request===reportRequest.current)setBusy(false);}}
  ...
  {classroom&&<section><div className="class-report-heading"><div><h2>{classroom.name}</h2><p>Join code: <code>{classroom.join_code}</code> · Share only with your students.</p></div><button disabled={busy} onClick={()=>void showReport(selected)}>Refresh report</button></div>{busy?<p role="status">Loading class results…</p>:<ClassroomInsights key={selected} students={report}/>}</section>}
