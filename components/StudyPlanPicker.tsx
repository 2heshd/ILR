...
export const planLabels:Record<PlanMode,string>={visual:'Flashcards',audio:'Flashcards',cloze:'Flashcards',reading:'Reading',listening:'Listening'};
export default function StudyPlanPicker({state,mode,onModeChange,onChange}:{state:StudyState;mode:PlanMode;onModeChange:(mode:PlanMode)=>void;onChange:(plan:StudyPlan)=>void}){
  ...
  return <section id="vocabulary-plans" className="study-plan card span-12">
    ...
    <div className="plan-mode-tabs" role="group" aria-label="Session skill">{(['visual','reading','listening'] as PlanMode[]).map((item)=><button key={item} aria-pressed={item==='visual'?['visual','audio','cloze'].includes(mode):mode===item} onClick={()=>onModeChange(item)}>{planLabels[item]}</button>)}</div>
